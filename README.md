...
```
* #### 5000+ id в одном запросе

_Батчинг на уровне сервиса_ — разбивать список на пачки по 500 и обрабатывать параллельно через CompletableFuture / ExecutorService.

_Bulk-lock_ через SELECT ... WHERE id IN (...) FOR UPDATE вместо построчного PESSIMISTIC_WRITE.

_Партиционирование таблицы documents по status_ — ускоряет локинг только нужных строк.

_Async API_ — принимать job, возвращать task-id, результаты отдавать отдельным эндпоинтом.
